...
extern "C" {
	...
	)
	{
		if (plugin->Properties.GetCount()) {
			*errorBuf="emRasFpPlugin: No properties allowed.";
			return NULL;
		}
		return new emImageFilePanel(
			parent,name,
			emRasImageFileModel::Acquire(
				parent.GetRootContext(),path
			)
		);
	}

	bool emRasFpPluginModelFunc(
	...
	)
	{
		if (
			...
			strcmp(className,"emRasImageFileModel")==0
		)
		{
			*pResult=emRasImageFileModel::Acquire(context,name,common);
			return true;
		}
		else {
			*errorBuf=emString::Format(
				"emRasFpPluginModelFunc: Unsupported class name: %s",
				className
			);
			return false;
		}
