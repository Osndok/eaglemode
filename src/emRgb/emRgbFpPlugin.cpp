...
extern "C" {
	...
	)
	{
		if (plugin->Properties.GetCount()) {
			*errorBuf="emRgbFpPlugin: No properties allowed.";
			return NULL;
		}
		return new emImageFilePanel(
			parent,name,
			emRgbImageFileModel::Acquire(
				parent.GetRootContext(),path
			)
		);
	}

	bool emRgbFpPluginModelFunc(
	...
	)
	{
		if (
			...
			strcmp(className,"emRgbImageFileModel")==0
		)
		{
			*pResult=emRgbImageFileModel::Acquire(context,name,common);
			return true;
		}
		else {
			*errorBuf=emString::Format(
				"emRgbFpPluginModelFunc: Unsupported class name: %s",
				className
			);
			return false;
		}
