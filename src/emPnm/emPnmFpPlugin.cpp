...
extern "C" {
	...
	)
	{
		if (
			...
			strcmp(className,"emPnmImageFileModel")==0
		)
		{
			*pResult=emPnmImageFileModel::Acquire(context,name,common);
			return true;
		}
		else {
			*errorBuf=emString::Format(
				"emPnmFpPluginModelFunc: Unsupported class name: %s",
				className
			);
			return false;
		}
