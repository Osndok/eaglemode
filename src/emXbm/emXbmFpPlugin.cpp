...
extern "C" {
	...
	)
	{
		if (
			...
			strcmp(className,"emXbmImageFileModel")==0
		)
		{
			*pResult=emXbmImageFileModel::Acquire(context,name,common);
			return true;
		}
		else {
			*errorBuf=emString::Format(
				"emXbmFpPluginModelFunc: Unsupported class name: %s",
				className
			);
			return false;
		}
