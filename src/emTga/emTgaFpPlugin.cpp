...
extern "C" {
	...
	)
	{
		if (plugin->Properties.GetCount()) {
			*errorBuf="emTgaFpPlugin: No properties allowed.";
			return NULL;
		}
		return new emImageFilePanel(
			parent,name,
			emTgaImageFileModel::Acquire(
				parent.GetRootContext(),path
			)
		);
	}

	bool emTgaFpPluginModelFunc(
	...
	)
	{
		if (
			...
			strcmp(className,"emTgaImageFileModel")==0
		)
		{
			*pResult=emTgaImageFileModel::Acquire(context,name,common);
			return true;
		}
		else {
			*errorBuf=emString::Format(
				"emTgaFpPluginModelFunc: Unsupported class name: %s",
				className
			);
			return false;
		}
